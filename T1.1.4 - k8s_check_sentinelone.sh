...
for FULLPOD in $PODS; do
  ...
  PROCESSES=$(kubectl exec -n "$NAMESPACE" "$POD" -- ps auxww | grep -E 's1-|sentinel' | grep -v -e grep -e "$0" || true)
...
echo
echo "[+] Detecting SentinelOne mode (KSPM / CWS / BOTH)..."

inventory_enabled=$(kubectl exec -n "$NAMESPACE_FILTER" "$HELPER_POD" -- printenv S1_INVENTORY_ENABLED 2>/dev/null || echo "")
inventory_only=$(kubectl exec -n "$NAMESPACE_FILTER" "$HELPER_POD" -- printenv S1_INVENTORY_ONLY 2>/dev/null || echo "")

MODE=""
if [[ "$inventory_enabled" == "true" && "$inventory_only" == "true" ]]; then
  MODE="KSPM"
elif [[ "$inventory_enabled" == "false" && "$inventory_only" == "false" ]]; then
  MODE="CWS"
elif [[ "$inventory_enabled" == "true" && "$inventory_only" == "false" ]]; then
  MODE="BOTH"
else
  MODE="UNKNOWN"
fi
